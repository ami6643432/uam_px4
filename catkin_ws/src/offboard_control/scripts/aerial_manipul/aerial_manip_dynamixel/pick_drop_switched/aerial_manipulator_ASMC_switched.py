...
class dynamixtest:
    ...
    def __init__(self,id):
        ...
        self.OPERATING_MODE          = 2 #Posn Mode 
    ...
    def get_motor_data(self):
        ...
        if self.PRESENT_POSITION/self.tick_ratio >= 150 or self.PRESENT_POSITION/self.tick_ratio <=-150:
            dxl_comm_result, dxl_error = self.packetHandler.write1ByteTxRx(self.portHandler, self.DXL_ID, self.ADDR_TORQUE_ENABLE, 0)
        # else:
        #     dxl_comm_result, dxl_error = self.packetHandler.write1ByteTxRx(self.portHandler, self.DXL_ID, self.ADDR_TORQUE_ENABLE, 1)
    ...
    def pid(self):
        ...
        self.error = self.PRESENT_POSITION - self.GOAL_POSITION
        
        p = self.error * self.kp
        
        #d = (self.error - self.previous_error) * self.kd
        d = ((self.PRESENT_VELOCITY*0.229*0.105*57.296)-self.GOAL_VELOCITY) * self.kd

        self.errInt += self.error*dt
        i = self.errInt*self.ki
        pd = -int(p+i+d)
        ...
        if self.DXL_ID == 1:
            print(self.DXL_ID,self.PRESENT_POSITION/self.tick_ratio)
...
def main(argv):
    ...
    obj0.set_goal_pos_callback(240) #motor 1 first
    # obj2.set_goal_pos_callback(70) #link 2 motor
    obj1.set_goal_pos_callback(140) #link 3
    obj3.set_goal_pos_callback(151) #gripper
