...
class Controller:
    ...
    def multiDoFCb(self, msg):
        ...
        print( "x: " , pt.transforms[0].translation.x)
        print( "y: " , pt.transforms[0].translation.y)
        ...
        # # (-0.5, 0)(approach position)
        if (pt.transforms[0].translation.x < -0.49 and pt.transforms[0].translation.x > -0.51) \
        and (pt.transforms[0].translation.y > -0.01 and pt.transforms[0].translation.y < 0.01 ):
            print("1. (-0.5, 0) reached")

            dynamixel_obj0.set_goal_pos_callback(180) #motor 1 first
            dynamixel_obj2.set_goal_pos_callback(135) #link 2 motor
            dynamixel_obj1.set_goal_pos_callback(145) #link 3
            dynamixel_obj3.set_goal_pos_callback(160) #gripper





        # # (-0.6,0)(gripping position)
        if (pt.transforms[0].translation.x < -0.59 and pt.transforms[0].translation.x > -0.61) \
        and (pt.transforms[0].translation.y > -0.01 and pt.transforms[0].translation.y < 0.01 ):
            print("1. (-0.6, 0) reached")
            ...
            # dynamixtest(0).set_goal_pos_callback(140) #motor 1 first
            # dynamixtest(2).set_goal_pos_callback(135) #link 2 motor
            # dynamixtest(1).set_goal_pos_callback(180) #link 3
            # dynamixtest(3).set_goal_pos_callback(160) #gripper    



        # # (-0.7,0)(gripping at position 1)
        if (pt.transforms[0].translation.x < -0.69 and pt.transforms[0].translation.x > -0.71) \
        and (pt.transforms[0].translation.y > -0.01 and pt.transforms[0].translation.y < 0.01 ):
            print("1. (-0.7, 0) reached")

            # dynamixtest(0).set_goal_pos_callback(140) #motor 1 first
            # dynamixtest(2).set_goal_pos_callback(135) #link 2 motor
            # dynamixtest(1).set_goal_pos_callback(180) #link 3
            
            dynamixel_obj3.set_goal_pos_callback(230)

            # dynamixtest(3).set_goal_pos_callback(225) #gripper 
        ...
        and (pt.transforms[0].translation.y > 0.09 and pt.transforms[0].translation.y < 0.11 ):
            ...
            # dynamixtest(0).set_goal_pos_callback(180) #motor 1 first
            # dynamixtest(2).set_goal_pos_callback(135) #link 2 motor
            # dynamixtest(1).set_goal_pos_callback(145) #link 3


        # (-0.5, 0.20)(drop 1 reached)
        if (pt.transforms[0].translation.x < -0.49  and pt.transforms[0].translation.x > -0.51) \
        and (pt.transforms[0].translation.y > 0.19 and pt.transforms[0].translation.y < 0.21 ):
            print("1. (-0.5, 0.2) drop 1 reached")
            # dynamixtest(0).set_goal_pos_callback(180) #motor 1 first
            # dynamixtest(2).set_goal_pos_callback(153) #link 2 motor
            # dynamixtest(1).set_goal_pos_callback(145) #link 3
    ...
    ## Object 2
        ...
        # # (-0.6,0.20)(gripping position)
        if (pt.transforms[0].translation.x < -0.59 and pt.transforms[0].translation.x > -0.61) \
        and (pt.transforms[0].translation.y > 0.19 and pt.transforms[0].translation.y < 0.21 ):
            print("2. (-0.6, 0.20) reached")
        ...
        # # (-0.7, 0.20)(gripping at position 1)
        if (pt.transforms[0].translation.x < -0.69 and pt.transforms[0].translation.x > -0.71) \
        and (pt.transforms[0].translation.y > 0.19 and pt.transforms[0].translation.y < 0.21 ):
            print("2. (-0.7, 0.20) reached")
        ...
        # (-0.6, 0.30))
        if (pt.transforms[0].translation.x < -0.59  and pt.transforms[0].translation.x > -0.61) \
        and (pt.transforms[0].translation.y > 0.29 and pt.transforms[0].translation.y < 0.31 ):
            print("2. (-0.6, 0.30) reached")
        ...
        # (-0.5, -0.40)(drop 2 reached)
        if (pt.transforms[0].translation.x < -0.49  and pt.transforms[0].translation.x > -0.51) \
        and (pt.transforms[0].translation.y > 0.39 and pt.transforms[0].translation.y < 0.41 ):
            print("1. (-0.5, 0.40) drop 2 reached")
            # dynamixtest(0).set_goal_pos_callback(180) #motor 1 first
            # dynamixtest(2).set_goal_pos_callback(153) #link 2 motor
            # dynamixtest(1).set_goal_pos_callback(145) #link 3
        ...
        # ( 0, 0.40)(Lock Position if there are only two objects)
        if (pt.transforms[0].translation.x < 0.01  and pt.transforms[0].translation.x > -0.01) \
        and (pt.transforms[0].translation.y > 0.39 and pt.transforms[0].translation.y < 0.41 ):
            print("End. ( 0, 0.60) Landing Position Reached")
            dynamixtest(0).set_goal_pos_callback(220) #motor 1 first
            dynamixtest(2).set_goal_pos_callback(70) #link 2 motor
            dynamixtest(1).set_goal_pos_callback(170) #link 3
    ...
    ## Object 3
            ...
            # # (-0.6,0.40)(gripping position)
        if (pt.transforms[0].translation.x < -0.59 and pt.transforms[0].translation.x > -0.61) \
        and (pt.transforms[0].translation.y > 0.39 and pt.transforms[0].translation.y < 0.41 ):
            print("3. (-0.6, 0.40) reached")
        ...
        # # (-0.7, 0.40)(gripping at position 1)
        if (pt.transforms[0].translation.x < -0.69 and pt.transforms[0].translation.x > -0.71) \
        and (pt.transforms[0].translation.y > 0.39 and pt.transforms[0].translation.y < 0.41 ):
            print("3. (-0.7, 0.40) reached")
            
            dynamixel_obj3.set_goal_pos_callback(225)



        # (-0.6, 0.50)) (Drop position)
        if (pt.transforms[0].translation.x < -0.59  and pt.transforms[0].translation.x > -0.61) \
        and (pt.transforms[0].translation.y > 0.49 and pt.transforms[0].translation.y < 0.51 ):
            print("3. (-0.6, 0.50) reached")
        ...
        # (-0.5, 0.60)(drop 3 reached)
        if (pt.transforms[0].translation.x < -0.49  and pt.transforms[0].translation.x > -0.51) \
        and (pt.transforms[0].translation.y > 0.59 and pt.transforms[0].translation.y < 0.61 ):
            print("3. (-0.5, 0.60) drop 3 reached")
            # dynamixtest(0).set_goal_pos_callback(180) #motor 1 first
            # dynamixtest(2).set_goal_pos_callback(153) #link 2 motor
            # dynamixtest(1).set_goal_pos_callback(145) #link 3
...
def main(argv):
    ...
    dynamixel_obj0.set_goal_pos_callback(220) #motor 1 first
    dynamixel_obj2.set_goal_pos_callback(70) #link 2 motor
    dynamixel_obj1.set_goal_pos_callback(170) #link 3
    time.sleep(1)
    dynamixel_obj3.set_goal_pos_callback(157) #gripper
    print("enabled2")
    ...
    # dynamixtest(0).set_goal_pos_callback(165) #motor 1 first
    # dynamixtest(1).set_goal_pos_callback(359) #gripper actuator motor
    # dynamixtest(2).set_goal_pos_callback(305) #link 2 motor

    # dynamixel_obj1.set_goal_pos_callback(180) #motor 1 first
    # dynamixel_obj2.set_goal_pos_callback(359) #gripper actuator motor
    # dynamixel_obj3.set_goal_pos_callback(310) #link 2 motor

    #gripper pick 359, drop 120
