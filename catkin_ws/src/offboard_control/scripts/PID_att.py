...
class Controller:
    ...
    def __init__(self):
        ...
        # Gains
        self.Kpos = np.array([-6, -6, -7])
        self.Kvel = np.array([-2, -2, -2])
        self.Kint = np.array([-0.1, -0.1, -1.0])
        self.norm_thrust_const = 0.065
        self.max_acc = 6.0
    ...
    def newPoseCB(self, msg):
        ...
        self.sp.pose.position.x = msg.pose.position.x        # if(self.magnet_ON):
        #     if (pt.transforms[0].translation.x < -0.48 and pt.transforms[0].translation.x > -0.52) \
        #     and (pt.transforms[0].translation.y > -0.52 and pt.transforms[0].translation.y < -0.48 ):            
        #         # GPIO.output(self.pin_1, True)
        #         #GPIO.output(self.pin_1, True)
    ...
    def geo_con(self):
        ...
        thrust = np.maximum(0.0, np.minimum(thrust, 0.95))
