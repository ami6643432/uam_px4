		...
		if(command_active_)
		    {
				...
				// Takeoff and lock ( locking is directly implemented in takeoff code )
				points.push_back(Eigen::Vector3d(0.0, -0.45, height));

				// Grab position (Approach position)
				points.push_back(Eigen::Vector3d(-0.5, -0.45, height));
				
				// Grab position (waypoint) 
				points.push_back(Eigen::Vector3d(-0.6, -0.45, height));
				
				// Grab position (Gripper close) 
				points.push_back(Eigen::Vector3d(-0.7, -0.45, height));

				// 
				points.push_back(Eigen::Vector3d(-0.6, -0.30, height));

				// Drop position -> Grab position (Gripper open)
				points.push_back(Eigen::Vector3d(-0.5, -0.10, height));

				// 
				points.push_back(Eigen::Vector3d(-0.6, -0.10, height));

				// Grab position (Gripper close)
				points.push_back(Eigen::Vector3d(-0.7, -0.10, height));

				// 
				points.push_back(Eigen::Vector3d(-0.6, 0.05, height));

				// Drop position -> Grab position (Gripper open)
				points.push_back(Eigen::Vector3d(-0.5, 0.25, height));


				// // Object 3	

				// 
				points.push_back(Eigen::Vector3d(-0.6, 0.25, height));


				// Grab position (Gripper close)
				points.push_back(Eigen::Vector3d(-0.7, 0.25, height));

				// 
				points.push_back(Eigen::Vector3d(-0.6, 0.35, height));

				// Drop position -> lock
				points.push_back(Eigen::Vector3d(-0.5, 0.35, height));

				// Land
				points.push_back(Eigen::Vector3d( 0.0, 0.35, height));
...
void start_trajectory(ros::Publisher trajectory_pub, std::vector<Eigen::Vector3d> points){
	...
	ros::Rate sleep_rate(100); //original at 100
	int i = 0;
